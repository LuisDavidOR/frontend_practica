...
const Categorias = () => {
  ...
  useEffect(() => { //el useEffect hace que cuando una vista es iniciada se ejecuta automaticamente
  ...
  const agregarCategoria = async () => {
    ...
    try {
      const respuesta = await fetch('http://127.0.0.1:3000/api/registrarcategoria', {
        method: 'POST',
        headers: {
  ...
  const eliminarCategoria = async () => {
    ...
    try {
      const respuesta = await fetch(`http://127.0.0.1:3000/api/eliminarcategoria/${categoriaAEliminar.id_categoria}`, {
        method: 'DELETE',
      });
  ...
  const actualizarCategoria = async () => {
    ...
    try {
      const respuesta = await fetch(`http://127.0.0.1:3000/api/actualizarcategoria/${categoriaEditada.id_categoria}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
