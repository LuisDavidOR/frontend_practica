...
const Ventas = () => {
  ...
  const obtenerVentas = async () => {
    try {
      const respuesta = await fetch('http://127.0.0.1:3000/api/obtenerventas'); // Ruta ajustada al controlador
      if (!respuesta.ok) {
  ...
  const obtenerDetalles = async (id_venta) => {
    ...
    try {
      const respuesta = await fetch(`http://127.0.0.1:3000/api/obtenerdetallesventa/${id_venta}`);
      if (!respuesta.ok) {
  ...
  const eliminarVenta = async () => {
    ...
    try {
      const respuesta = await fetch(`http://127.0.0.1:3000/api/eliminarventa/${ventaAEliminar.id_venta}`, {
        method: 'DELETE',
      });
